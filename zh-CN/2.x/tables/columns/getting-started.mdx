...
要在用户Session中持久化表格或单列搜索，可以在Livewire组件中重写 `shouldPersistTableSearchInSession()` 或 `shouldPersistTableColumnSearchInSession()` 方法：
...
> 如果你想要在后面面板之外使用tooltips提示，请先在应用中[安装`@ryangjchandler/alpine-tooltip`](https://github.com/ryangjchandler/alpine-tooltip#installation)，引入[`tippy.css`](https://atomiks.github.io/tippyjs/v6/getting-started/#1-package-manager)。如果你使用[自定义后台主题](/docs/zh-CN/2.x/admin/appearance#创建主题)，你也需要安装[`tippy.css`](https://atomiks.github.io/tippyjs/v6/getting-started/#1-package-manager)。
