...
我们使用异步 Alpine 来加载 Alpine 组件，因此我们需要将 `x-ignore` 属性添加到加载我们组件的 div。我们也需要将 `ax-load` 属性添加到该 div，用以告知 Alpine 加载我们的组件。你可以到本文档的[核心概念](/docs/3.x/support/assets#异步-alpinejs-组件)章节中了解更多相关信息。
