...
`getData()` 方法用于返回一个包含数据集和标签的数组。每个数据集都是一个标签化数组用来绘制图表的point，标签为字符串。Filament 用来渲染图表的这种数据结构，与[Chart.js](https://www.chartjs.org/docs)图表库是一致的。你可以借助[Chart.js 文档](https://www.chartjs.org/docs)去了解 `getData()` 方法基于图表类型可能的返回值。
