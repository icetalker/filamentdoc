...
使用 Filament 之前，你需要先熟悉 Laravel。Filament 是基于许多 Laravel 核心概念构建的，特别是[数据库迁移](https://laravel.com/docs/migrations)和 [Eloquent ORM](https://laravel.com/docs/eloquent)。如果你没此前没用过 Laravel，或者需要复习，建议你跟着 [Laravel Bootcamp](https://bootcamp.laravel.com/blade/installation) 去创建一个小应用。那个教程涵盖了创建 Laravel 应用的基础知识。
...
> 你页可以使用 Filament 的 [Action](../actions/modals#modal-forms) 去打开模态框表单，在患者表单中直接创建、编辑或删除治疗。
...
此外，我们需要 `price` 字段。我们可以使用文本输入框，并进行一些定制工作使之适于货币输入。可以用 `numberic()` 方法，添加验证的同时，改变移动设备中键盘的布局。同时可以添加一个前缀 `prefix('€')`，使之在输入框签名渲染 `€`，而不会改变字段保存的输出。
