...
要去注册自定义页面，将全限定类名添加到服务提供者的 `$styles` 属性数组中。你应该使用唯一名称作为键名，指向样式表的URL 作为值。
...
要去注册自定义页面，将全限定类名添加到服务提供者的 `$scripts` 属性数组中。你应该使用唯一名称作为键名，指向脚本的URL作为值。这些脚本会被添加到 Filament 核心脚本之后。
