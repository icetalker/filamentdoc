...
不过，使用该技术，对于在[后台面板](../panels)中使用插件的用户，可能会额外的复杂度。如果他们使用的[自定义主题](../panels/theming)，由于他们最终都要编译自己的 CSS 文件，这倒没什么；但是，如果他们使用面板自带的默认样式，你需要留意你插件视图中使用的样式类。比如，如果你使用了一个默认样式表中没有包含的类，用户也没有自己编译，它就不会包含在最终的 CSS 文件中。这意味着你的插件视图可能不会如你期望的那样展示。这是我推荐在插件中编译及[注册](#注册-css-文件) Tailwind CSS 编译后样式表的情形之一。
