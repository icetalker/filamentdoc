...
所有的布局组件([Grid](../../forms/layout/grid#grid-component)、[Section](../../forms/layout/section), [Fieldset](../../forms/layout/fieldset)、[Section](../../forms/layout/section) 等)都有一个 `relationship()` 方法。使用该方法时，在该布局内的所有字段会被保存到关联模型中，而非所有者的模型： 
...
关联管理器是 Livewire 组件。当它们首次加载时，拥有者的记录(作为父级模型——即主资源模型——的 Eloquent 记录)在公用的 `$ownerRecord` 属性上挂载。因此，你可以这样访问拥有者记录：

```php
$this->ownerRecord
```

不过，在静态方法，如 `form()` 或 `table()` 中，无法访问 `$this`。因此，你可以[使用回调函数](../../forms/advanced#form-component-utility-injection)获取 `$livewire` 示例：
...
您可能决定希望资源表单及表格与关系管理器的表单及表格相同，然后希望重用以前编写的代码。这很容易，只需在关联管理器中调用资源的 `form()` 和 `table()` 方法：
