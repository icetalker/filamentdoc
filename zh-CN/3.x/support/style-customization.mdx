...
您可能会想将内部 Blade 视图发布到应用程序中，以便进行自定义。我们不建议这样做，因为它会在将来的更新中对您的应用程序进行破坏性更改。请尽可能使用 [CSS 钩子类](#将样式应用到钩子类中)。

如果您确实要发布这类 Blade 视图，请在 `composer.json` 中将所有 Filament 包锁定到特定版本，然后通过碰撞版本号手动更新 Filament，在每次更新后测试整个应用程序。这将帮助您安全地识别破坏性更改。
