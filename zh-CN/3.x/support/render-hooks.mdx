...
- `panels::page.end` - 页面内容容器结尾，也[可将范围锁定到](#渲染钩子作用域)页面或者资源类
...
- `panels::page.start` - 页面内容容器开始处，也[可将范围锁定到](#渲染钩子作用域)页面或者资源类
...
- `panels::styles.after` - 样式定义之后
- `panels::styles.before` - 样式定义之前
...
- `panels::user-menu.profile.after` - [用户菜单](../panels/navigation#自定义用户菜单)的 Profile 项之后
- `panels::user-menu.profile.before` - [用户菜单](../panels/navigation#自定义用户菜单)的 Profile 项之前
