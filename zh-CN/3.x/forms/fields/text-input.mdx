...
- 当使用 `readOnly()` 时，该字段仍然会在表单提交时发送给服务器。使用浏览器控制台或者通过 JavaScript，仍然可以修改它的值。可以使用 [`dehydrated(false)`](advanced#阻止字段被-dehydrated) 阻止该行为。
- 使用 `readOnly()` 时，样式不会该变，比如不会减少透明度。T
