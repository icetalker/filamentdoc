...
默认图标大小是 `IconEntrySize::Large`，不过你也可以将其大小自定义为 `IconEntrySize::ExtraSmall`、`IconEntrySize::Small`、`IconEntrySize::Medium`、`IconEntrySize::ExtraLarge` 或 `IconEntrySize::TwoExtraLarge`：
...
> 如果模型类中的字段已经被强制转换为 `bool` 或 `boolean`，Filament 可以检测到此，你无需手动调用 `boolean()`。
