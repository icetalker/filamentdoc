...
某些情况下，在表单提交之前模型实例不可用。比如，新建 post 的表单：在提交表单之前，post模型的实例不会被传到表单里。
...
某些情况下，在表单提交之前模型实例不可用。比如，在新建 post 的表单中，提交表单前post的实例不会被传到表单form里。这种情况下，你需要[传模型类](#注册模型类)代替，不过，关联字段就需要随后手动进行保存了。
