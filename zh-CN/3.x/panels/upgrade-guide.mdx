...
> **Livewire v3 is 仍处于 Beta 测试阶段！**<br />
...
由于 Livewire v3 仍处于 Beta 测试阶段，请在 `composer.jison` 中将 `minimum-stability` 设置为 `dev`：
...
由于 Livewire v3 仍处于 Beta 测试阶段，请在 `composer.json` 中将 `minimal-stability` 设置为 `dev`：
...
在 v2 中，你可以使用 `/resources/views/vendor/filament/components/brand.blade.php` 文件自定义面板的 Logo。在 v3 中，这个文件被重命名为 `/resources/views/vendor/filament-panels/components/logo.blade.php`。
