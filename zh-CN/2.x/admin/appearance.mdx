...
在 `config/filament.php` 中，将 `layouts.max_content_width` 设置成 `xl` 到 `7xl` 之间的值，或者没有最大宽度的 `full` :
...
在 `config/filament.php` 文件中，将 `layouts.notifications.alignment` 设置为 `left`、`center` 或 `right` 中的任意值，将 `layouts.notifications.vertical_alignment` 设置为 `top`、`center` 或者 `bottom` 中的任意值:
