...
- `forms::components.tags-input.delete-button` - Tag 输入框中用来删除 Tag 的按钮
- `forms::components.wizard.completed-step` - 向导卡中的完成步骤

### 表格构造器中的图标别名

- `tables::columns.collapse-button`
- `tables::filters.remove-button` - 删除一个过滤器的按钮
- `tables::filters.remove-all-button` - 删除所有过滤器的按钮
- `tables::grouping.collapse-button` - 用于折叠一组记录的按钮
- `tables::header-cell.sort-asc` - 以升序排列的排序按钮
- `tables::header-cell.sort-desc` - 以降序排列的排序按钮
- `tables::pagination.previous-button` - 上一页按钮，用于移动端及"simple pagination"
- `tables::pagination.next-button` - 下一页按钮，用于移动端及"simple pagination"
...
- `notifications::database.modal.pagination.previous-button` - 数据库通知模态框上一页按钮
- `notifications::database.modal.pagination.next-button` - 数据库通知模态框下一页按钮
