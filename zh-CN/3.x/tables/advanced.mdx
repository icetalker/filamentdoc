...
首先，找到Livewire组件。如果您使用的是面板构造器中的资源，并且希望向 List 页面添加简单的分页，则需要打开资源中的`Pages/List.php` 文件，而不是资源类本身。
...
要显示全局搜索输入，表中至少有一列需要是 `searchable()` 可搜索的。或者，如果您使用 Scout 来控制哪些列已经可以搜索，您可以简单地将 `searchable()` 传递给整个表：
