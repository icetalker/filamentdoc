...
Filament 的信息列表可以使用 [Action](../actions)。Action 是可以添加到任何信息列表组件的按钮。同时，你也可以单独地[渲染匿名 Action 集](#adding-anonymous-actions-to-an-infolist-without-attaching-them-to-a-component)，而不必将其附到特定的信息列表组件。
