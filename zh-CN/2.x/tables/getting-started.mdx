...
默认情况下，表格是分页的。要禁用此功能，你可以在Livewire组件上重写 `isTablePaginationEnabled()` 方法： 
...
如果你要为一个特定的字段[重写URL](columns/getting-started#打开URL)，或者在点击字段时[运行Livewire操作](columns#执行操作)，请查看[表格字段文档](columns#打开URL)。

### 记录 action

另外，你也可以配置表格行，用以触发action，替代打开URL:
...
Livewire 使用了一个在 UR L查询字符串中存储数据的特性，用来获取跨请求数据。
