...
> 如果你的表格包含 ENUM 字段，`doctrine/dbal` 包无法扫描表格且会崩溃。因此，如果包含了 ENUM 字段，Filament 无法为你的导出器生成字段。更多该问题的详细信息可查看[此处](https://github.com/doctrine/dbal/issues/3819#issuecomment-573419808)。
...
### 计算导出列的状态Calculated export column state
...
本例中，`users` 是用于检测是否存在的关联名。列名必须是 `users_exists`，`avg()`, `max()`, `min()` and `sum()`。
...
默认情况下，导出文件将会被上传到[配置文件](../installation#发布配置)定义的存储磁盘中。你也可以设置 `FILAMENT_FILESYSTEM_DISK` 环境变量对此进行修改。
...
现在，您可以通过将 `$options` 参数注入到任何闭包函数中，在导出器(Exporter)类内中访问这些选项数据。例如，你可能希望在`formatStateUsing()` 中使用它来[格式化列的值]（#格式化导出列的值）：
