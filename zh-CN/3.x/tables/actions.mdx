...
Filament 表格可以使用 [Action](../actions/installation)。Action 是添加到[表格行末](#行-action)或表格[头部](#头部-action)的按钮。比如，如果你想在表格头部添加"新建"记录的 Action，在每行中添加"编辑"和"删除" Action。[批量 Action](#批量操作)可用在记录选中时执行代码。另外，Action 也可以添加到[表格列](#列-action)，这样该列中的每个单元格都是 action 的触发器。
