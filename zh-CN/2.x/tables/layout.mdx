...
类似于字段，你可以根据浏览器的响应式[临界点](https://tailwindcss.com/docs/responsive-design#overview)隐藏一个stack。使用 `visibleFrom()` 方法：
...
你甚至可以控制每个组件在各个临界点[breakpoint](https://tailwindcss.com/docs/responsive-design#overview)中所消耗的Grid列数：
...
如果你不是在后台面板中使用表格，你可能会发现折叠或者打开内容时，并没有动画效果。你可以安装 [Alpine.js Collapse Plugin](https://alpinejs.dev/plugins/collapse) 来启用动画：
