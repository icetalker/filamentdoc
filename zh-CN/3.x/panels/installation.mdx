...
你也可以考虑在部署脚本中运行 `php artisan filament:cache-componengs`，特别是当你有大量组件(资源、页面、Widgets、关联管理器、自定义 Livewire 组件等)时。这将在应用的 `bootstrap/cache/filament` 目录中生成缓存文件，其中包含每个组件类型的索引。这在某些应用中可以显著提升 Filament 的性能，因为它减少了需要扫描的文件数量并且自动发现组件。
...
部署是，你可以考虑使用 `php artisan icons:cache` 缓存图标，Filament 使用的是 [Blade Icons](https://blade-ui-kit.com/blade-icons) 包，缓存后将更具性能。不过，这非常依赖于硬件，许多服务器不需要此命令 Filament 就能顺利运行。
