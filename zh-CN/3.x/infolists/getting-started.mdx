...
本指南将引导您了解使用 Filament 的信息列表包构建信息列表的基本知识。如果你打算在自己的 Livewire 组件中添加一个新的信息列表，你应该[先添加](adding-an-infolist-to-a-livewire-component)再回来。如果你正在向[应用资源](../panels/resources/getting-started)或另一个 Filament 包添加信息列表，你就可以开始了！

## 定义 Entry

创建信息列表的第一步是，定义会在列表中展示的条目(Entry)。你可以在调用 `Infolist` 对象中 `entries()` 方法实现。该方法接收一个 Entry 对象数组。
